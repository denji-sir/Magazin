...
export function RegisterPage() {
  ...
  return (
    <Box py={80} bg="var(--color-bg)" style={{ minHeight: 'calc(100vh - var(--header-height))' }}>
      <Container size={500}>
        <Title ta="center" style={{ fontFamily: '"Playfair Display", serif' }}>Создать аккаунт</Title>
        <Text c="dimmed" fz="sm" ta="center" mt={5}>
          Уже есть аккаунт? <Anchor size="sm" component={Link} to="/auth/login" color="gold">Войти</Anchor>
        </Text>
        ...
        <Paper withBorder shadow="md" p={30} mt={30} radius="lg">
          <form onSubmit={form.onSubmit(handleSubmit)}>
            <Stack>
              ...
              <PasswordInput label="Пароль" placeholder="Минимум 6 символов" required {...form.getInputProps('password')} />
              <PasswordInput label="Подтверждение пароля" placeholder="Повторите пароль" required {...form.getInputProps('confirmPassword')} />
              <Button fullWidth mt="xl" type="submit" color="dark" loading={loading} size="md">Зарегистрироваться</Button>
            </Stack>
          </form>
        </Paper>
      </Container>
