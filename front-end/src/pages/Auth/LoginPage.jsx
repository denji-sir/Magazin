...
export function LoginPage() {
  ...
  const handleSubmit = async (values) => {
    setLoading(true);
    try {
      // Mock login for now since backend is not ready
      // In real scenario: await login(values.email, values.password);
    ...
    <Box py={100} bg="var(--color-bg)" style={{ minHeight: 'calc(100vh - var(--header-height))' }}>
      <Container size={420}>
        ...
        <Text c="dimmed" fz="sm" ta="center" mt={5}>
          Нет аккаунта?{' '}
          <Anchor size="sm" component={Link} to="/auth/register" color="gold">
            Зарегистрироваться
          </Anchor>
        </Text>
